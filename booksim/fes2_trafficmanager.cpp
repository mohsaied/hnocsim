...
#endif
					if (!dest_buf->IsAvailableFor(cf->vc)) {
					  cout << "node = " << n << endl;
	...
	for(int subnet = 0; subnet < _subnets; ++subnet) {
		for(int n = 0; n < _nodes; ++n) {
			...
			if(iter != flits[subnet].end()) {
                ...
                ++creds_o[n][f->vc];
                //if(creds_o[n][f->vc] != creds_n[n][f->vc]+1)
                //    cout << "CHECK NODE " << n << ", VC " << f->vc << endl;
                //cout << _time << " | (" << n <<"," << f->vc <<") tm_old->" << creds_o[n][f->vc] << endl;	
#ifdef TRACK_FLOWS
...
void FeS2TrafficManager::writeActivityToFile( )
{
  // delete old activity files
  of2.close();
  of3.close();
  remove("link_utilization.out");
  remove("topo_map.out");
  
  // create new activity files
  of2.open("link_utilization.out");
  of3.open("topo_map.out");
